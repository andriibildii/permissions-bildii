...
import {Table, TableBody, TableCell, tableCellClasses, TableContainer, TableHead, TableRow, IconButton} from '@mui/material';
...
export const PermissionsTable = ({permissions}) => {
    ...
    const columns = [
        {
            ...
            label: 'User Name',
            minWidth: 250,
            button: <IconButton onClick={sortData}><BsSortDown/></IconButton>
        },
    ...
    const rows = newPermissions.map(todo => createData(
        <CircleIcon/>,
        todo.name,
        <input type='checkbox' checked={todo.view} onChange={() => handleChange('view', todo.id)}/>,
        <input type='checkbox' checked={todo.execute}
               onChange={() => handleChange('execute', todo.id)}/>,
        <input type='checkbox' checked={todo.modify}
               onChange={() => handleChange('modify', todo.id)}/>,
        <input type='checkbox' checked={todo.del} onChange={() => handleChange('del', todo.id)}/>,
    ...
    return (
        <>
            <div>
                <TableContainer sx={{maxHeight: 620, width: '100%', boxShadow: 'none'}}>
                                ...
                                <TableRow hover key={row.name}>
                                    <StyledTableCell component="th" scope="row" style={{paddingLeft: 10}}>
                                        <div className='userName_container'>
                                            <span className='userIcon'>{row.icon}</span><span
                                            className='userName'>{row.name}</span>
                                        </div>
                                    </StyledTableCell>
                                    <StyledTableCell align="center">{row.view}</StyledTableCell>
                                    <StyledTableCell align="center">{row.execute}</StyledTableCell>
                                    <StyledTableCell align="center">{row.modify}</StyledTableCell>
                                    <StyledTableCell align="center">{row.del}</StyledTableCell>
                                    <StyledTableCell align="right">{row.dropdownMenu}</StyledTableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
